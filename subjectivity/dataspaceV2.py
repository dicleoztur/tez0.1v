...
geziyakurdi'deki documentsimilarity'den aldım.
@author: dicle
...
class Corpus:
    ...
    def __init__(self, cname="", rootpath=""):
        ...
        self.matrixpath = IOtools.ensure_dir(self.rootpath + os.sep + "matrix" + os.sep)
    ...
    def read_corpus(self, inputpath, labelwisefileidlist, plaintext=False, nostopwords=True):
        dateroots = []
        datePOStag = []
        docPOStag = []
        docroots = []
    ...
    def get_docterm_matrix(self):
        ...
        doctermframe.to_csv(self.matrixpath+os.sep+self.corpusname+"doctermFREQ.csv")
        return doctermframe
        
        
    def compute_tfidf3(self):
        ...
        doctermframe = pd.DataFrame(matrix, index = self.cfd_DocRoot.conditions(), columns=self.cfd_RootDoc.conditions()) 
        doctermframe.to_csv(self.matrixpath+os.sep+self.corpusname+"doctermTFIDF.csv")
        return doctermframe
    
    
    def compute_tfidf(self):
        freqdfpath = self.matrixpath + os.sep + self.corpusname+"doctermFREQ.csv"
        tfidfdfpath = self.matrixpath + os.sep + self.corpusname+"doctermTFIDF.csv"
        return metrics.compute_tfidf(freqdfpath, tfidfdfpath)
    ...
    def extract_features(self):
        ...
        freqdf = pd.read_csv(self.matrixpath+os.sep+self.corpusname+"doctermFREQ.csv", index_col=0)
        docs = freqdf.index.values.tolist()
        abstractwords = keywordhandler.get_abstractwords()
        
        featurematrix1 = pd.DataFrame(index=docs, columns=["abstractness", "adjratio", "advratio", "label"])
        featurematrix2 = pd.DataFrame(index=docs, columns=abstractwords)
        
        # take only the count of abstract words
        for doc in docs:
            abscount = 0
            for absword in abstractwords:
                try:
                    count = freqdf.loc[doc, absword]
                    abscount = abscount + count
                except:
                    count = 0
            featurematrix1.loc[doc, "abstractness"] = abscount / len(list(self.cfd_DocRoot[doc]))       
            featurematrix1.loc[doc, "label"] = self.doclabeldct[doc]
        
        # take the tfidf weights of all abtsract lexicon in the corpus        
        tfidfdf = pd.read_csv(self.matrixpath+os.sep+self.corpusname+"doctermTFIDF.csv", index_col=0)
        for absword in abstractwords:
            try:
                wordvector = tfidfdf.loc[:, absword]
                wordvector = wordvector.values
                featurematrix2[absword] = wordvector
            except:
                featurematrix2[absword] = np.zeros(len(docs))
        
        
        
        # adjective ratio
        adjratio = self.POStag_features(self.cfd_DocPOStag, ['ADJ'], ['Noun'])
        advratio = self.POStag_features(self.cfd_DocPOStag, ['ADV'], ['ADJ', 'Verb'])
        
        
        print adjratio[0]
        for doc, val in adjratio:
            print doc,"   - ",docs[0]
            featurematrix1.loc[doc, "adjratio"] = val
        for doc, val in advratio:
            featurematrix1.loc[doc, "advratio"] = val  
        
        featurematrix2["adjratio"] = featurematrix1.loc[:, "adjratio"]
        featurematrix2["advratio"] = featurematrix1.loc[:, "advratio"]
        featurematrix2["label"] = featurematrix1.loc[:, "label"]
        
        featurematrix1.to_csv(self.rootpath+os.sep+"featurematrix1.csv")
        featurematrix2.to_csv(self.rootpath+os.sep+"featurematrix2.csv")
...
if __name__== "__main__":
    ...
    '''
    1. construct dataspace read corpus
    2. find tfidf. find doc similarity
    3. compute features
    4. cluster and classify
    5. compare doc similarities on visuals.
...
#    bunlari 13 eylul'de kapattim. geziyakurdi icin calistirdiklarimdi.
#    
